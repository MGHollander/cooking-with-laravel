    ...
    <DefaultLayout>
        ...
        <div class="mb-4 px-2 sm:px-0 flex justify-between items-center">
            <Button :href="route('users.create')"
                    class="px-2 text-sm bg-indigo-600 text-sm no-underline hover:bg-indigo-800 hover:text-white">
                Gebruiker toevoegen
            </Button>
                        ...
                        <tr v-for="user in users.data" :key="user.id">
                            ...
                            <td class="px-4 py-5 sm:px-6 whitespace-nowrap text-right text-sm font-medium">
                                <Link :href="route('users.edit', user.id)"
                                      class="text-indigo-600 hover:text-indigo-900 no-underline hover:underline">
                                    Bewerk
                                </Link>
                            </td>
                        </tr>
