...
<template>
  ...
  <DefaultLayout>
    <template #header>{{ $t('users.create.title') }}</template>
    ...
    <form class="mx-auto max-w-2xl" @submit.prevent="submit">
      ...
      >
        <Button class="text-xs" type="submit" :disabled="form.processing">{{ $t('users.create.save') }}</Button>
      </div>
    </form>
  </DefaultLayout>
</template>
