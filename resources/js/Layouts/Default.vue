        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="flex h-16 justify-between">
            ...
            <div v-if="$page.props.auth.user" class="hidden sm:ml-6 sm:flex sm:items-center">
              <div class="relative ml-3 flex items-center space-x-1">
                ...
                <Dropdown align="left" width="48">
                  ...
                  <template #content>
                    <DropdownLink
                      :href="route(`recipes.create.${$page.props.locale}`)"
                      :active="route().current(`recipes.create.${$page.props.locale}`)"
                    >
                      {{ $t("nav.add_recipe") }}
                    </DropdownLink>

                    <DropdownLink
                      :href="route(`import.index.${$page.props.locale}`)"
                      :active="route().current(`import.index.${$page.props.locale}`)"
                    >
                      {{ $t("nav.import_recipe") }}
                    </DropdownLink>
                  </template>
                ...
                <Dropdown align="right" width="48">
                  ...
                  <template #content>
                    ...
                    <DropdownLink
                      :href="route(`users.settings.edit.${$page.props.locale}`)"
                      :active="route().current(`users.settings.edit.${$page.props.locale}`)"
                    >
                      {{ $t("nav.settings") }}
                    </DropdownLink>
                    ...
                    <DropdownLink :href="route(`logout.${$page.props.locale}`)" method="post" as="button">{{
                      $t("nav.logout")
                    }}</DropdownLink>
                  </template>
                </Dropdown>
            ...
            <div v-else class="flex space-x-1">
              <NavLink
                :href="route(`login.${$page.props.locale}`)"
                :active="route().current(`login.${$page.props.locale}`)"
                >{{ $t("nav.login") }}</NavLink
              >
            </div>
        ...
        <div v-if="$page.props.auth.user" class="sm:hidden">
          ...
          <div v-if="showNav === 'user'">
            ...
            <div class="space-y-1 border-t border-gray-200 py-1">
              ...
              <ResponsiveNavLink
                :href="route(`logout.${$page.props.locale}`)"
                method="post"
                as="button"
