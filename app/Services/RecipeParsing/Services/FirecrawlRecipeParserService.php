    ...
    public function parse(string $url): ?ParsedRecipeData
    {
        ...
        try {
            ...
            if (!($data['success'] ?? false)) {
                ...
                throw new RecipeParsingException(
                    message: 'Firecrawl API returned unsuccessful response',
                    url: $url,
                    parser: $this->getName()
                );
            }
            ...
            if (!$parsedData->isValid()) {
                $missingFields = [];
                if (empty($parsedData->title)) $missingFields[] = 'title';
                if (empty($parsedData->ingredients)) $missingFields[] = 'ingredients';
                if (empty($parsedData->steps)) $missingFields[] = 'steps';
